...
function error-message {
	if [ $? = 1 ]; then
		echo "Error!" 2> >(logger -t $(basename "$0") -i -p error.warning)
	fi
...
function displayhelp {
	echo "I don't know what I'm doing here!"
}
...
function cleanup {
	rm /tmp/*.$$
	logger -t `basename "$0"` -i -p user.info -s “Temp files are being cleaned up and exiting.”
	exit 1
}
trap cleanup SIGINT
